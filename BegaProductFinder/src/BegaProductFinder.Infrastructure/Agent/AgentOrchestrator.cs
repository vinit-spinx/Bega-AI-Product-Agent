...
public sealed class AgentOrchestrator : IAgentOrchestrator
{
    ...
    {
        // Multi-group form: requests=[{group, query}, ...]. Falls back to a single-entry list
        // built from the top-level query/group fields when requests is omitted, so a simple
        // one-group call still works exactly as before.
        var requestsArray = input["requests"]?.AsArray();
        List<GroupSearchRequest> requests;
        if (requestsArray is { Count: > 0 })
        {
            requests = requestsArray
                .Select(n => n as JsonObject)
                .Where(o => o is not null)
                .Select(o => new GroupSearchRequest(
                    o!["group"]?.GetValue<string>() ?? string.Empty,
                    o["query"]?.GetValue<string>() ?? string.Empty))
                .Where(r => r.Group.Length > 0 && r.Query.Length > 0)
                .ToList();
        }
        else
        {
            var fallbackGroup = input["group"]?.GetValue<string>();
            var fallbackQuery = input["query"]?.GetValue<string>() ?? string.Empty;
            requests = fallbackGroup is not null
                ? [new GroupSearchRequest(fallbackGroup, fallbackQuery)]
                : [new GroupSearchRequest(string.Empty, fallbackQuery)];
        }

        if (requests.Count == 0 || requests.All(r => r.Query.Length == 0))
            return ("{\"results\":[]}", null);

        // Shared filters apply uniformly across every group in `requests`. Group/query are
        // intentionally excluded here — each request supplies its own.
        var sharedFilters = new ProductSearchFilters
        {
        ...
        // Total budget across ALL requests combined, not per group. Cap at 6.
        var totalTopK = Math.Min(input["top_k"]?.GetValue<int>() ?? 6, 6);

        // Note: no zero-trust CCT/control-protocol post-filter here (there used to be one). It
        // would silently strip products that ProductSearchService's tiered relaxation kept
        // deliberately — e.g. the closest Bollard match when nothing has the exact requested
        // CCT — which would contradict the per-group RelaxedFilters/Note that already accounts
        // for exactly this and reports it honestly instead of hiding it.
        var outcomes = await _productSearch.SearchMultiGroupAsync(requests, sharedFilters, totalTopK, ct);
        var allProducts = outcomes.SelectMany(o => o.Products).ToList();

        // Compact JSON for Claude's context, grouped per request so it can see exactly what was
        // found (or not found, or relaxed) for EACH group — this is what lets the model report
        // honestly ("no In-grade option under $500") instead of silently substituting or
        // fabricating. Full data goes to the frontend via sseChunk.
        var breakdown = outcomes.Select(o => new
        {
            group = o.Group,
            note = o.Note,
            relaxed_filters = o.RelaxedFilters.Count > 0 ? o.RelaxedFilters : null,
            products = o.Products.Count > 0 ? CompactProductsRaw(o.Products) : null
        });
        var json = JsonSerializer.Serialize(new { results_by_group = breakdown }, _jsonOptions);
        var sseChunk = allProducts.Count > 0
            ? new AgentStreamChunk { Type = AgentStreamEventType.Products, Products = allProducts }
            : null;
    ...
    private static string CompactProductsJson(List<ProductSearchResult> products) =>
        JsonSerializer.Serialize(CompactProductsRaw(products), _jsonOptions);

    /// <summary>
    /// The same compact per-product projection as <see cref="CompactProductsJson"/>, but as a raw
    /// enumerable rather than a pre-serialized string — used when nesting product summaries
    /// inside a larger JSON structure (e.g. the per-group breakdown in <see cref="SearchProductsAsync"/>).
    /// </summary>
    private static IEnumerable<object> CompactProductsRaw(List<ProductSearchResult> products) =>
