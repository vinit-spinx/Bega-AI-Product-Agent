...
import { fetchOverviewV2, type OverviewData, type TimeRange } from '@/services/insights/insightsV2Service';
...
export default function OverviewTab() {
  ...
  const radarRef  = useGSAPScrollReveal();
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Activity chart + Opportunity Radar */}
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-5">
        ...
        <div ref={radarRef} className="bg-white border border-bega-border-1 rounded-2xl p-5">
          <p className="text-[13px] font-semibold text-bega-text-1 mb-1">Opportunity Radar</p>
          <p className="text-[11px] text-bega-text-3 mb-4">Growing specification trends · impact score</p>
          <OpportunityRadar items={data?.opportunityRadar ?? []} loading={loading} />
        </div>
      </div>
    </div>
  );
